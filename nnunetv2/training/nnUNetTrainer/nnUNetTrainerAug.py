...
from nnunetv2.training.data_augmentation.custom_transforms.random_slice_dropout import RandomSliceDropoutTransform
from nnunetv2.training.data_augmentation.custom_transforms.save_transform import DebugSaveTransform
from nnunetv2.training.data_augmentation.custom_transforms.random_dim import RandomDimInsideMaskTransform
from nnunetv2.training.data_augmentation.custom_transforms.random_cuboid_dropout import RandomCuboidDropoutTransform
...
class nnUNetTrainer(object):
    ...
    def get_training_transforms(
        ...
        transforms = []
        transforms.append(DebugSaveTransform("/data7/samhitar/nnUNet/aug", "00_raw_img", max_images=3))
        if do_dummy_2d_data_aug:
            ignore_axes = (0,)
        ...
        transforms.append(DebugSaveTransform("/data7/samhitar/nnUNet/aug", "00_initial", max_images=3))

 
        transforms.append(
        RandomSliceDropoutTransform(
        axis=2,  # 2=Z if shape is (B, C, Z, Y, X)
        p_per_sample=0.3,
        max_slices_to_drop=3,
        fill_value=0.0
        )
        )
        
       # transforms.append(DebugSaveTransform("/data7/samhitar/nnUNet/aug", "01_slice_dropout", max_images=3))
        
        
        transforms.append(
        RandomCuboidDropoutTransform(
        p_per_sample=0.3,
        max_cuboids=2,
        max_size_ratio=0.3,  # relative to patch size
        fill_value=0.0
        )
        )
       
        #transforms.append(
        #DebugSaveTransform("/data7/samhitar/nnUNet/aug", "02_cuboid_dropout", max_images=3)
        #)

        
        
        transforms.append(
        RandomDimInsideMaskTransform(
        min_factor=0.3,
        max_factor=1.0,
        p_per_sample=0.3,
        mask_idx_in_seg=0,
        ...
        transforms.append(
        DebugSaveTransform("/data7/samhitar/nnUNet/aug", "03_cell_dim", max_images=3)
        )
        ...
        transforms.append(RandomTransform(
            ...
            ), apply_probability=0.3 #0.1
        ))
        
        #transforms.append(
        #DebugSaveTransform("/data7/samhitar/nnUNet/aug", "04_gaussian_noise", max_images=3)
        #)
        ...
        transforms.append(RandomTransform(
            ...
            ), apply_probability=0.2 #0.2
        ))
        
        
        #transforms.append(DebugSaveTransform("/data7/samhitar/nnUNet/aug", "05_gaussian_blur", max_images=3))
        ...
        #transforms.append(DebugSaveTransform("/data7/samhitar/nnUNet/aug", "06_random_transform", max_images=3))
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        if self.label_manager.has_regions:
            ...
            #print('code without softmax')
            # no need for softmax
            output_seg = output.argmax(1)[:, None]
            #print(output_seg.shape)
            #print(output.shape)
            #tf.imwrite("/data7/samhitar/nnUNet/nnUNet_results/Dataset027_biggerkernel/nnUNetTrainer__nnUNetPlans__3d_fullres_k11/fold_0/output.tif",output)
            #tf.imwrite("/data7/samhitar/nnUNet/nnUNet_results/Dataset027_biggerkernel/nnUNetTrainer__nnUNetPlans__3d_fullres_k11/fold_0/output_seg.tif",output_seg)
    ...
    def on_epoch_end(self):
        ...
        # save non-overwriting checkpoints every N epoch

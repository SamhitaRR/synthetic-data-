...
class nnUNetDataLoader(DataLoader):
    ...
    def generate_train_batch(self):
        ...
        save_dir = '/data7/samhitar/nnUNet/nnUNet_results/Dataset027_biggerkernel/nnUNetTrainer__nnUNetPlans__3d_fullres/fold_0'
        ...
        if self.transforms is not None:
            with torch.no_grad():
                with threadpool_limits(limits=1, user_api=None):
                    ...
                    #tf.imwrite(os.path.join(save_dir, 'data_all.tif'), data_all_np)
                    ...
                    for b in range(self.batch_size):
                        ...
                        seg_np = tmp['segmentation'][0].cpu().numpy()
                        print(len(tmp['segmentation']))
                        print([i.shape for i in tmp['segmentation']])
                        data_all_np = data_all.cpu().numpy()
                        tf.imwrite(os.path.join(save_dir, 'data_all.tif'), data_all_np)
                        
                        filename = os.path.join(save_dir, f'seg_{b}.tif')
                        tf.imwrite(filename, seg_np)
                        
                    data_all = torch.stack(images)
                    #print(f'data_all shape after for loop: {data_all.shape}')
                    if isinstance(segs[0], list):
                        seg_all = [torch.stack([s[i] for s in segs]) for i in range(len(segs[0]))]
                        #print(f'seg_all length after for loop in if: {len(seg_all)}')
                    else:
                        seg_all = torch.stack(segs)
                        #print(f'seg_all shape after for loop in else: {seg_all.shape}')
                    del segs, images
